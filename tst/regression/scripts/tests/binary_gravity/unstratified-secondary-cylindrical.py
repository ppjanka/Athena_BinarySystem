"""
ununstratified-secondary-cylindrical.py
...
def run(**kwargs):
    ...
    arguments = ['job/problem_id=unstratified-secondary-cylindrical',
                 'output1/file_type=hdf5',
                 'output1/variable=prim',
                 'output1/ghost_zones=false',
                 'output1/dt=%.20f' % (0.015),
                 'time/cfl_number=0.3',
                 'time/tlim=%.20f' % (1.5),
                 'mesh/nx1=32',
                 'mesh/x1min=0.1',
...
def analyze():
    ...
    initial_state = athena_read.athdf('bin/unstratified-secondary-cylindrical.out1.00000.athdf')
    ...
    for i in range(1,101):
        test_results = athena_read.athdf('bin/unstratified-secondary-cylindrical.out1.%05i.athdf' % i)
        ...
        acc = np.sum(rho * vol * (1.-GM) / ((1.-x)**2 + y**2)) / np.sum(rho*vol)

        dt = time - time_curr

        # calculate where the CoM should be
        rtot = np.sqrt((1.-com_curr[0])**2 + com_curr[1]**2)
        ...
        print("[unstratified-secondary-cylindrical]: error_tot_com = %.2e (max %.2e)" % (error_tot_com, 0.05*time))
