"""
coordCenterRotation_centeredSphere-cylindrical.py
...
def run(**kwargs):
    ...
    arguments = ['job/problem_id=coordCenterRotation_centeredSphere-cylindrical',
...
def analyze():
    ...
    initial_state = athena_read.athdf('bin/coordCenterRotation_centeredSphere-cylindrical.out1.00000.athdf')
    r, rho_expected = [initial_state[key] for key in ['x1f', 'rho']]

    # check if the frames are identical
    for i in range(1,5):
        test_results = athena_read.athdf('bin/coordCenterRotation_centeredSphere-cylindrical.out1.0000%i.athdf' % i)
        rho = test_results['rho']

        error_rel_rho = np.sum(np.abs(rho_expected - rho)) / np.sum(rho)
        print("[coordCenterRotation_centeredSphere-cylindrical]: error_rel_rho = %.2e" % error_rel_rho)
