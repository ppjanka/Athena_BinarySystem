"""
coordCenterRotation_offCenterSphere-spherical_polar.py
...
Current test:
 -- a sphere off-center wrt the rotation axis is properly advected
...
def run(**kwargs):
    ...
    arguments = ['job/problem_id=coordCenterRotation_offCenterSphere-spherical_polar',
...
def in_sphere (r, theta, phi, time):
    dist = np.sqrt(r*r + 0.3*0.3 - 2.*r*np.sin(theta)*0.3*np.cos(phi+time))
    return dist < 0.2

def analyze():

    initial_state = athena_read.athdf('bin/coordCenterRotation_offCenterSphere-spherical_polar.out1.00000.athdf')
    ...
    for i in range(1,5):
        test_results = athena_read.athdf('bin/coordCenterRotation_offCenterSphere-spherical_polar.out1.0000%i.athdf' % i)
        rho = test_results['rho']
        time = test_results['Time']

        rho_expected = np.ones(rho.shape) * 1.
        rho_expected[np.where(np.sqrt(r*r + 0.3*0.3 - 2.*r*np.sin(theta)*0.3*np.cos(phi+time)) < 0.2)] = 2.
        ...
        print("[coordCenterRotation_offCenterSphere-spherical_polar]: error_rel_rho = %.2e" % error_rel_rho)
