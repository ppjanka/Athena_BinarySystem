"""
barycenterRotation_offCenterSphere2-cylindrical.py
...
def run(**kwargs):
    ...
    arguments = ['job/problem_id=barycenterRotation_offCenterSphere2-cylindrical',
...
def analyze():
    ...
    initial_state = athena_read.athdf('bin/barycenterRotation_offCenterSphere2-cylindrical.out1.00000.athdf')
    ...
    if phi_init > np.pi:
        phi_init_bary = 2.*np.pi - phi_init_bary
    print("r_init_bary = ", r_init_bary, ", phi_init_bary = ", phi_init_bary)

    # check if the sphere is correctly advected
    for i in range(1,5):
        test_results = athena_read.athdf('bin/barycenterRotation_offCenterSphere2-cylindrical.out1.0000%i.athdf' % i)
        ...
        print("[barycenterRotation_offCenterSphere2-cylindrical]: error_rel_rho = %.2e" % error_rel_rho)
