"""
barycenterRotation_offCenterSphere-spherical_polar.py
...
def run(**kwargs):
    ...
    arguments = ['job/problem_id=barycenterRotation_offCenterSphere-spherical_polar',
...
def analyze():
    ...
    initial_state = athena_read.athdf('bin/barycenterRotation_offCenterSphere-spherical_polar.out1.00000.athdf')
    ...
    for i in range(1,5):
        test_results = athena_read.athdf('bin/barycenterRotation_offCenterSphere-spherical_polar.out1.0000%i.athdf' % i)
        ...
        rho_expected[np.where(np.sqrt(r**2 + r_exp**2 - 2.*r*np.sin(theta)*r_exp*np.cos(phi-phi_exp)) < 0.1)] = 2.
        ...
        print("[barycenterRotation_offCenterSphere-spherical_polar]: error_rel_rho = %.2e" % error_rel_rho)
